...
describe Arachni::HTTP::Client do
    ...
    before( :all ) do
        ...
        Rack::Handler::ArachniIntrospector.run_in_thread Client, {
            Host: 'stuff'
        }
    ...
    describe '#custom_404?' do
        ...
        context "when the 404 cache exceeds #{described_class::CUSTOM_404_CACHE_SIZE} entries" do
            it 'it is pruned as soon as possible' do
                expect(subject._404_cache).to be_empty
                ...
                expect(subject._404_cache.size).to eq described_class::CUSTOM_404_CACHE_SIZE
            end
        end
    end
        ...
        context 'when the page has been fingerprinted for a custom 404 handler' do
            context 'and it has a custom handler' do
                it 'returns false' do
                    ...
                    expect(subject.checked_but_not_custom_404?( path )).to be_falsey
                end
            end
            ...
            context 'and it does not have a custom handler' do
                it 'returns true' do
                    ...
                    expect(subject.checked_but_not_custom_404?( subject.get_path( @url ) )).to be_truthy
                end
            end
        end
    ...
    describe '#checked_for_custom_404?' do
        ...
        context 'when the page has been fingerprinted for a custom 404 handler' do
            context 'and it has a custom handler' do
                it 'returns true' do
                    ...
                    expect(subject.checked_for_custom_404?( url )).to be_truthy
                end
            end
        ...
        context 'when the page has not been fingerprinted for a custom 404 handler' do
            it 'returns false' do
                expect(subject.checked_for_custom_404?( url )).to be_falsey
            end
        end
    end
    ...
    describe 'needs_custom_404_check?' do
        context 'when #checked_for_custom_404?' do
            context false do
                before(:each) { allow(subject.instance).to receive(:checked_for_custom_404?) { false } }

                it 'returns true' do
                    expect(subject.needs_custom_404_check?( @url )).to be_truthy
                end

                context 'and #checked_but_not_custom_404?' do
                    context false do
                        before(:each) { allow(subject.instance).to receive(:checked_but_not_custom_404?) { false } }
                    ...
                    context true do
                        before(:each) { allow(subject.instance).to receive(:checked_but_not_custom_404?) { true } }
            ...
            context true do
                before(:each) { allow(subject.instance).to receive(:checked_for_custom_404?) { true } }
                ...
                context 'and #checked_but_not_custom_404?' do
                    context true do
                        ...
                        it 'returns false' do
                            expect(subject.needs_custom_404_check?( @url )).to be_falsey
                        end
                    end
