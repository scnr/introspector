...
class Rack::Handler::SCNRIntrospector

Rack::Handler.register :scnr_introspector, self

class Server
    def initialize( options = {}, &block )
        SCNR::Engine::HTTP::Client.service(options, &block )
    end
    ...
    def stop
        SCNR::Engine::HTTP::Client.remove_service
        true
    end

    def running?
        SCNR::Engine::HTTP::Client.has_service?
    end
end
    ...
    def service( response )
        ...
        environment = {
            ...
            'QUERY_STRING'    => request.effective_parameters.
                map { |k, v| "#{SCNR::Engine::Link.encode(k)}=#{SCNR::Engine::Link.encode(v)}" }.join('&'),
        ...
        if request.body.is_a? Hash
            body = request.body.map { |k, v| "#{SCNR::Engine::Form.encode(k)}=#{SCNR::Engine::Form.encode(v)}" }.join('&')
        else
            body = SCNR::Engine::Form.encode(request.body.to_s)
        end
        ...
        begin
            ...
            if @options[:trace]
                request.with_trace( @options[:trace], &app_call )
            else
                app_call.call
            end
